...
class YUVReader(VideoReader):
    def __init__(self, src_path, width, height, src_format='444', skip_frame=0):
        super().__init__(src_path, width, height)
    ...
    def read_one_frame(self, dst_format="444"):
        if self.eof:
            return self._none_exist_frame(dst_format)
        y = self.file.read(self.y_size)
        uv = self.file.read(self.uv_size)
        ...
        uv = np.frombuffer(uv, dtype=np.uint8).copy().reshape(2, self.height, self.width)
        y = y.astype(np.float32) / 255
        uv = uv.astype(np.float32) / 255

        return self._get_dst_format(y=y, uv=uv, src_format='444', dst_format=dst_format)
